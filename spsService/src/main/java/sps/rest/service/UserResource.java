...
public class UserResource {
	...
	@QueryParam("showGS") boolean showGameSessions;
	@QueryParam("showGSQeustions") boolean showGameSessionQuestions;
	...
	public Response retrieveUserById(@PathParam("userId") long userId) {
		...
		userReplyType = MappingUtils.mapUserToUserReply(userReplyType, user, request.getRequestURI(),request.getSession().getId(),false,false,false, showGameSessions);
	...
	public Response retrieveGameSessionByUserAndId(@PathParam("userId") long userId, @PathParam("gameSessionId") long gameSessionId) {
		UserReplyType userReplyType = null;
		UsersUtil uu = new UsersUtil(entityManager);
		...
		if(gameSessionUser != null) {
			//Map the user to a user reply type
			userReplyType = MappingUtils.mapUserToUserReply(userReplyType, user, request.getRequestURI(),request.getSession().getId(),false,false,false,false);
			//Map the Game Session type
			GameSessionType gameSessionType = MappingUtils.mapGameSessionToGameSessionType(gameSessionUser.getGameSession(), false, false);
			List<Answer> answers = au.findAnswersByGameSessionUser(gameSessionUser);
			for(Answer answer:answers) {
				gameSessionType = MappingUtils.mapAnswerToGameSessionType(answer, gameSessionType);
			}		
	...
	public Response retrieveTransactions(@PathParam("userId") long userId) {
		...
		//Map the user to a user reply type
		userReplyType = MappingUtils.mapUserToUserReply(userReplyType, user, request.getRequestURI(),request.getSession().getId(),true,true,true,showGameSessions);
		entityManager.getTransaction().commit();
		entityManager.close();
		//Add the user reply type to the reply
		getReply().getUserReply().add(userReplyType);
	...
	private Response retrieveUserByEmail(String email) {
		...
		//Map the user to a user reply type
		userReplyType = MappingUtils.mapUserToUserReply(userReplyType, user, request.getRequestURI(),request.getSession().getId(),true,true,false, showGameSessions);
		entityManager.getTransaction().commit();
		entityManager.close();
		//Add the user reply type to the reply
		getReply().getUserReply().add(userReplyType);
	...
	public Response postUser(SpsRequest fullRequest) {
		...
		if(action.equalsIgnoreCase(ACTION_CREATE)) {
			...
			userReply = MappingUtils.mapUserToUserReply(userReply, user, request.getRequestURI(),request.getSession().getId(), true, true, false, showGameSessions);
			entityManager.getTransaction().commit();
			entityManager.close();
			...
			} else if(userRequest.getUser() != null && userRequest.getUser().getEmail() != null && userRequest.getUser().getEmail().length() > 0) {
		...
		return Response.status(406).entity("Action: " + action + " is not a vlid action besed on incoming request").build();
	...
	public Response postTransaction(SpsRequest fullRequest, @PathParam("userId") long userId) {
		...
		if(action == null)
		{
			return	Response.status(406).entity("Query Parameter Action is null and should not be").entity(fullRequest).build();
		}    	
		...
		if(action.equalsIgnoreCase(ACTION_CREATE)) {
			...
			entityManager.getTransaction().begin();
			Users user = uu.findUser(userId);
			ShareTransaction transaction = MappingUtils.mapTransactionTypeToTransaction(userRequest.getShareTransaction(), user, null);
		...
		return Response.status(406).entity("Action: " + action + " is not a valid action besed on incoming request").build();
	...
	public Response addAnswerToGameSession(SpsRequest spsRequest, @PathParam("userId") long userId, @PathParam("gameSessionId") long gameSessionId, @PathParam("questionId") long questionId) {
		...
		au.addAnswer(answer, question, gameSessionUser);
	
		gameSessionType = MappingUtils.mapGameSessionToGameSessionType(gameSession, false, false);
		entityManager.getTransaction().commit();
		entityManager.close();
		//Add the user reply type to the reply
	...
	@Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
	@Path("/{userId}")
	public Response updateUser(@PathParam("userId") long userId, @FormParam("email") String email, @FormParam("password") String password) {
		UserReplyType userReplyType = null;
		UsersUtil uu = new UsersUtil(entityManager);
		Users user = null;
